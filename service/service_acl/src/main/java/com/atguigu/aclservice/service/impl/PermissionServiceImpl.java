...
//        });
    ...
    //把返回所有菜单list集合进行封装的方法，设置顶层菜单及其下属菜单
    ...
    private static Permission selectChildren(Permission permissionNode, List<Permission> permissionList) {
        ...
        //2 遍历所有菜单list集合，进行判断比较，比较顶层菜单id和集合中猜的的pid值是否相同
    ...
    //2 根据当前菜单id(当前菜单id是递归查询入口)，查询菜单里面子菜单id，封装到list集合
    private void selectPermissionChildById(String id, List<String> idList) {
        //查询菜单里面子菜单id
        QueryWrapper<Permission> wrapper = new QueryWrapper<>();
        wrapper.eq("pid", id);
        wrapper.select("id");//指定查出指定列，只查出该列的值
        List<Permission> childIdList = baseMapper.selectList(wrapper);
        //把childIdList里面菜单id值获取出来，封装idList里面，做递归查询
        childIdList.stream().forEach(item -> {//JDK8基础中的语法，流加Lambda表达式
            //封装idList里面
            idList.add(item.getId());
            //递归查询
